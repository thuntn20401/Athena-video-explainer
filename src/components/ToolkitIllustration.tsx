...
export const ToolkitIllustration: React.FC<{ width: number; startFrame?: number }> = ({ width, startFrame = 0 }) => {
  ...
  const sp = (delay: number, cfg?: object) =>
    spring({ frame: f - delay, fps, config: { damping: 14, ...cfg } });

  // Background fade
  const bgOp = interpolate(f, [0, 12], [0, 1], { extrapolateRight: "clamp" });

  // Shadow scale in
  const shadowProg = sp(5, { damping: 16 });
  const shadowScale = interpolate(shadowProg, [0, 1], [0, 1]);

  // Character — spring up (main element, first)
  const charProg = sp(0, { damping: 14 });
  const charY = interpolate(charProg, [0, 1], [50, 0]);
  const charOp = interpolate(f, [0, 10], [0, 1], { extrapolateRight: "clamp" });

  // Lines — fade in after character
  const linesOp = interpolate(f, [10, 22], [0, 1], { extrapolateRight: "clamp" });

  // Gears — spring with rotation
  const gearsProg = sp(5, { damping: 12 });
  const gearsScale = interpolate(gearsProg, [0, 1], [0, 1]);
  const gearsRot = interpolate(gearsProg, [0, 1], [-30, 0]);

  // Speech bubbles — spring pop
  const speechProg = sp(6, { damping: 10 });
  const speechScale = interpolate(speechProg, [0, 1], [0, 1]);

  // Tools — staggered spring pop
  const calcProg   = sp(8,  { damping: 13 });
  const alarmProg  = sp(10, { damping: 13 });
  const locProg    = sp(12, { damping: 13 });
  const camProg    = sp(14, { damping: 13 });
  const envProg    = sp(16, { damping: 13 });
  const clkProg    = sp(18, { damping: 13 });
  const calProg    = sp(20, { damping: 13 });
  const lockProg   = sp(22, { damping: 13 });

  const calcScale  = interpolate(calcProg,  [0, 1], [0, 1]);
  const alarmScale = interpolate(alarmProg, [0, 1], [0, 1]);
  const locScale   = interpolate(locProg,   [0, 1], [0, 1]);
  const camScale   = interpolate(camProg,   [0, 1], [0, 1]);
  const envScale   = interpolate(envProg,   [0, 1], [0, 1]);
  const clkScale   = interpolate(clkProg,   [0, 1], [0, 1]);
  const calScale   = interpolate(calProg,   [0, 1], [0, 1]);
  const lockScale  = interpolate(lockProg,  [0, 1], [0, 1]);

  return (
    <svg viewBox="0 0 500 500" width={width} height={width} style={{ overflow: "visible" }}>
      {/* Background */}
      <g opacity={bgOp} dangerouslySetInnerHTML={{ __html: G_BG }} />

      {/* Shadow */}
      <g transform={`scale(${shadowScale})`} style={{ transformOrigin: "235px 440px" }} dangerouslySetInnerHTML={{ __html: G_SHADOW }} />

      {/* Dashed lines */}
      <g opacity={linesOp} dangerouslySetInnerHTML={{ __html: G_LINES }} />

      {/* Gears */}
      <g transform={`scale(${gearsScale}) rotate(${gearsRot})`} style={{ transformOrigin: "138px 68px" }} dangerouslySetInnerHTML={{ __html: G_GEARS }} />

      {/* Speech bubbles */}
      <g transform={`scale(${speechScale})`} style={{ transformOrigin: "250px 65px" }} dangerouslySetInnerHTML={{ __html: G_SPEECH }} />

      {/* Calculator */}
      <g transform={`scale(${calcScale})`} style={{ transformOrigin: "419px 287px" }} dangerouslySetInnerHTML={{ __html: G_CALCULATOR }} />

      {/* Alarm */}
      <g transform={`scale(${alarmScale})`} style={{ transformOrigin: "425px 199px" }} dangerouslySetInnerHTML={{ __html: G_ALARM }} />

      {/* Locator */}
      <g transform={`scale(${locScale})`} style={{ transformOrigin: "418px 123px" }} dangerouslySetInnerHTML={{ __html: G_LOCATOR }} />

      {/* Camera */}
      <g transform={`scale(${camScale})`} style={{ transformOrigin: "361px 67px" }} dangerouslySetInnerHTML={{ __html: G_CAMERA }} />

      {/* Envelope */}
      <g transform={`scale(${envScale})`} style={{ transformOrigin: "168px 119px" }} dangerouslySetInnerHTML={{ __html: G_ENVELOPE }} />

      {/* Clock */}
      <g transform={`scale(${clkScale})`} style={{ transformOrigin: "86px 133px" }} dangerouslySetInnerHTML={{ __html: G_CLOCK }} />

      {/* Calendar */}
      <g transform={`scale(${calScale})`} style={{ transformOrigin: "61px 193px" }} dangerouslySetInnerHTML={{ __html: G_CALENDAR }} />

      {/* Padlock */}
      <g transform={`scale(${lockScale})`} style={{ transformOrigin: "57px 265px" }} dangerouslySetInnerHTML={{ __html: G_PADLOCK }} />

      {/* Character — main element */}
      <g opacity={charOp} transform={`translate(0, ${charY})`} dangerouslySetInnerHTML={{ __html: G_CHARACTER }} />
    </svg>
  );
};